...
- **Repo-root, durable across features**, markdown: `CONTEXT.md`, `docs/adr/`, `.out-of-scope/`, `.not-yet-specified.md`. Owners: `/al-grill-adr` (CONTEXT + domain ADRs), `/al-steer` (out-of-scope, and grooming `.not-yet-specified.md` — the deferred-question ledger `/al-grill-adr` and `/al-design` append to, `/al-refine` scans before speccing; entries graduate to a decision or move to `.out-of-scope/`, never silently absorbed). The writing skills run the inline document-integrity check (`references/doc-integrity.md`) on `CONTEXT.md` and domain ADR writes before handoff; `.out-of-scope/` and `.not-yet-specified.md` are outside the document gate.
