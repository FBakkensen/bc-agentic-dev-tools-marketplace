...
When the last user/API-facing technical task lands, `/al-implement` opens the slice verify task from `blocked` to `ready`. `/al-refine` writes the `Verification Plan` and flips to `ready-for-verification`. `/al-code-review` runs per-slice; a clean review stamps `review=clean` on the verify task. If its `/grill-me` auto-loop materializes new technical tasks, `/al-refine` writes fresh proof before `/al-implement` resumes and `/al-code-review` re-runs.
