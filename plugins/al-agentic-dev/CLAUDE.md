...
| `subagents/` (folder) | plugin-level | spawnable subagent prompt blocks (was the `agents/` folder): `al-red-green.md` (one AAA case RED→GREEN), `al-review-lens.md` + `al-review-lens-bc.md` (one focused review pass); spawned by `/al-implement`, `/al-code-review`, `/al-refactor`; self-contained (ship to consumer repos); each names its Claude model per `model-selection.md` |
...
| `model-selection.md` | plugin-level | which Claude model a spawned subagent runs on: the three tiers (`sonnet`/`opus`/`fable`), intelligence > taste > cost, bulk→`sonnet`, escalate-if-below-bar, and the review carve-out (decomposed lenses + GPT veto run on `sonnet` on purpose); the **single home** the subagent blocks and spawning skills (`/al-implement`, `/al-build`, `/al-mutate`, `/al-refactor`, `/al-code-review`) point at instead of restating the tier |
...
├── model-selection.md           # Which Claude model a spawned subagent runs on: tiers + escalate-if-below-bar + review carve-out; single home the subagent blocks and spawning skills point at
...
├── CONTEXT.template.md
├── adr.template.md
├── subagents/                   # Spawnable subagent prompt blocks (was agents/): self-contained, Claude model per model-selection.md
