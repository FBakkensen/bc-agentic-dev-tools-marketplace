...
When the last user/API-facing technical task lands, `/al-implement` opens the slice verify task from `blocked` to `ready`. `/al-refine` writes the `Verification Plan` and flips to `ready-for-verification`. `/al-code-review` runs per-slice, **report-only by default**: it spawns the review-lens subagents (`references/subagents/al-review-lens.md`, `al-review-lens-bc.md`), dedups, adversarially judges, cross-family-vets via `/al-second-opinion`, then reports the must-fix queue (each routed to `/al-implement T-NNN`), the nits, and the gate decision — the user drives fixes. `--fix` additionally lands the must-fix findings in-loop by spawning the red-green subagent per finding, commits under the originating `T-NNN`, and **re-reviews once** (not an unbounded loop). A clean review stamps `review: clean`. Escalation classes (replan-class finding, a fix that won't go green, a recurrence, a `needs-a-decision` finding) route to `/al-steer`, writing nothing durable.
