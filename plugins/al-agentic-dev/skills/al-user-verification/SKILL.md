...
- Inline partial-run record present → resume from the next unasked check. No partial-run record → start from the first check. Status is `ready-for-verification` in both cases; the inline record is the differentiator.
...
- **A human is present.** The walk is user-driven; there is no agent-driven substitute. **Autonomous seat** (selected per `${CLAUDE_SKILL_DIR}/../../references/autonomy-seat.md` — the turn was initiated by the goal evaluator, not a human): never run the walk — there is no walker. `/al-autopilot` parks with a stop report *before* this skill, so an autonomous invocation is a routing error; return the state to the caller. Degraded verification never flips `done`.
...
**Spawn #2 (contract check + guided walk).** Only on pre-flight green. `new-agent-container.ps1` → `publish-apps.ps1` → run Contract examples through their named client/harness (agent-run, no user involvement), then hand the user the Web Client URL (`http://<container-name>/BC/`) plus credentials per the Preconditions login grant and guide the walk one instruction at a time. Symmetric with spawn #1's first two primitives; no replay step (batch already greened on spawn #1). Evidence is the transcript of the user's verbatim answers plus any screenshots the user pastes.
...
Announce verify task: `T-NNN` ID, slice slug, counts by scope (`E2E`, `Contract`, `Exploration`), estimated walk length (*"6 steps, ~5 minutes"*), first example. Hand over the Web Client URL, credentials, and a deep link to the starting page (`http://<container>/BC/?page=<id>`, page id read from the page AL) — the user's first action lands on the right screen, not hunting Tell Me. Leave status `ready-for-verification` while driving checks. One example open at a time; running three in parallel loses failure context when one goes red.
...
First functional fail in any E2E/Contract example, or functional failure discovered during Exploration: stop the walk/check. Do not continue to later steps in the same example, do not move to later examples. Ask the user for a screenshot at the failure moment — the one point where visual evidence pays; `/al-steer` triages from it. Record inside the task block:

- Which example (`V#`, `C#`, or `X#`) and which step/check/prompt.
- Observed vs expected, verbatim from the user's report (or captured client output), with any screenshot referenced.
...
Session interrupted mid-walk leaves the verify task at `ready-for-verification` with a partial record inline (which example/check the run reached). Humans get interrupted far more than agents; this is the normal case, not the exception. Re-entering this skill on the same `ready-for-verification` verify task resumes from the next unasked check; do not restart from the first example, do not make the user re-walk greens. Inline record is the resume point.

## Gate event

Once when the verify task flips to `done`. Gate report names slice (slug + `event-model.md` step), what the user confirmed in BC vocabulary (Role action, Business Event, View state, Status value, API/client result), the usability findings surfaced (→ candidate tasks), the evidence (transcript + any screenshots), the second-opinion outcome (reconciled / skipped), next handoff: `/al-refine` on first technical task of next slice; or, if this was the last slice, **open the `kind=breaking-change` task `blocked` → `ready`** (its `Depends on:` this verify task is now satisfied — this skill is its named flip-owner) and hand off to `/al-validate-breaking-changes`, then `/al-code-review` per-feature.
