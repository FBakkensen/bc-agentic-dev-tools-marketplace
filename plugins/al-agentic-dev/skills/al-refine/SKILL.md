...
- Scan `.not-yet-specified.md` at repo root when present — the deferred-question ledger `/al-steer` grooms. An entry the task's behaviour touches means the spec would encode a guess on a question the user already flagged as undecided: **Stop**, name the entry, route to `/al-steer` (or `/al-grill-adr` for a domain rule) to graduate it first. No entry touches the task → proceed; the scan costs one read.
