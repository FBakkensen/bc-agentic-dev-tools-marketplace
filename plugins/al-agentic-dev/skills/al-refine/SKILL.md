...
- Scan `.not-yet-specified/*.md` at repo root when present — the deferred-question ledger `/al-steer` grooms. A question the task's behaviour touches means the spec would encode a guess on a question the user already flagged as undecided: **Stop**, name the question file, route to `/al-steer` (or `/al-grill-adr` for a domain rule) to graduate it first. No question touches the task → proceed; the scan costs one folder read.
