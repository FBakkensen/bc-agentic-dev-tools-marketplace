...
Slot-fill, not free prose. Each slot is sharpened by a gate: when earned, when skipped. Names are the citation; rationale lives in ADRs (when load-bearing) or in the conversation transcript that produced this doc.
...
`## Goal` → `## Problem` → `## Solution` → `## Slice(s) (Event Modeling)` → `## Module map` → `## R → P → W` → (optional `## Module dependencies`, `## Flow`) → `## Brownfield touchpoints` → `## Test strategy` → `## ADRs cited` → (optional `## Future composition`) → `## Risks`.

Every section uses the gate format: a short slot-fill, with `_When earned:_` / `_Skip when:_` triggers stating what makes the section honest. No em-dashes anywhere in the generated file; see `voice-contract.md`.
...
- No: *"Implement a new settlement export feature that, when a sales invoice is posted, will take the relevant data from the posted document and produce a bank-file payload..."*
...
Wrap the paragraph in a CAUTION callout.
...
- No: *"There is currently no automated way to export posted invoice data to the bank, which means that finance users have to manually copy the totals..."*
...
Wrap the paragraph in a TIP callout.
...
- No: *"We will build a new module called `settlement-export` that subscribes to the `OnAfterPostSalesDoc` event published by codeunit `Sales-Post`, extracts the posted document into a payload..."*
...
One paragraph per slice. Each slice opens with its **pattern** (`Command` / `Automation` / `Translation` / `View`) and fills five positions: **trigger → command → event → state → view**. Pattern catalogue and BC trigger-source mapping live in `LANGUAGE.md` *Slice* entry.
...
- No: *"User-facing slice, when something happens on the sales side, the system processes it and updates configuration data..."*
...
**Sharpness, slice completeness gate.** Every slice fills all five positions. A void means the feature is not ready for `/al-scope`. Voids and what they mean:
...
Bold the Status column values (`**new**`, `**changed**`, `**extended**`); leave `kept` plain. Linkify ADR mentions in the Role column as `[ADR-NNNN](../../docs/adr/NNNN-slug.md)`.

| Module | Status | Pattern | Role |
|---|---|---|---|
| `src/{module-a}/` | **new** | Façade | {one phrase, the responsibility, in CONTEXT.md vocabulary, with ADR cites linkified} |
| `src/{module-b}/` | **extended** | Generic Method | {one phrase} |
| `src/{module-c}/` | touched | - | {one phrase, what is added or changed at the seam} |

One row per module. Status is `new` / `changed` / `extended` / `touched` / `kept`. Pattern is one entry from `bc-patterns.md`, or `-` for trivial touchpoints. Role is one phrase in domain vocabulary, never a procedure signature.
...
Mermaid `graph LR` between codeunits. Names match Module map exactly. Arrow labels only when the verb is not obvious. Complements the Flow sequence diagram (runtime sequence) with a static dependency view.

```
graph LR
    A[SettlementExportMgt] --> B[SettlementBatchSvc]
    B --> C[SettlementLineEmitter]
```
...
When both diagrams earn their place, group them under `## Diagrams` with `### Module dependencies` and `### Flow` subsections. When neither earns its place, the section is absent; no empty slot. Mermaid only. Maximum one of each.
...
- [ADR-NNNN](../../docs/adr/NNNN-slug.md), {one phrase on relevance to this feature}.
...
{Content describing how a future feature is expected to compose on the current architecture.}
...
One alert per risk class, severity matched to actual risk. Each alert leads with `**Label**:` then body.

| Class | Severity | Use this alert |
|---|---|---|
| AppSource compatibility / informational | low | `> [!NOTE]` |
| Mitigated breaking change (e.g. shielded by overload) | medium | `> [!TIP]` |
| Real data-loss or unmitigated breaking risk | high | `> [!WARNING]` (or `> [!CAUTION]` when destructive) |
