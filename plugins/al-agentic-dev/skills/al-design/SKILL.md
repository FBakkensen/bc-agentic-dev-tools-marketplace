...
2. **Repo memory.** Read `CONTEXT.md` (materialise from template if missing). Read every ADR in `docs/adr/` touching the area. Use the project's domain language throughout the rest of this flow. **Absorb every cited ADR's Consequence section (where present) into a slot of `architecture.md`** (Module map, Brownfield touchpoints, R→P→W, Test strategy) — `## ADRs cited` is a pointer list, not a substitute. Downstream skills do not re-read the ADR.
...
| `${CLAUDE_SKILL_DIR}/../../references/adr.template.md` | `docs/adr/NNNN-<slug>.md` | step 10, on ADR accept; `NNNN` = next free 4-digit number |
| `${CLAUDE_SKILL_DIR}/references/architecture.template.md` | `specs/<NNN>-<slug>/architecture.md` | step 11 |
