...
Use `references/architecture.template.md`. Required sections in order: `## Goal`, `## Problem` (wrap in `> [!CAUTION]`), `## Solution` (wrap in `> [!TIP]`), `## Slice(s) (Event Modeling)`, `## Module map`, `## R → P → W`, `## Brownfield touchpoints`, `## Test strategy`, `## ADRs cited`, `## Risks` (one severity-matched alert per risk class). Optional after `## R → P → W`: `## Module dependencies` (Mermaid `graph LR`, gated per template triggers) and / or `## Flow` (Mermaid `sequenceDiagram` or `flowchart TD`, gated per template triggers); max one of each. Optional after `## ADRs cited`: `## Future composition` wrapped in a default-closed `<details>` block when a forward-looking composition seam earns its place. Bold the Status column in `## Module map`. Linkify ADR mentions in Role / R→P→W / ADRs cited as `[ADR-NNNN](../../docs/adr/NNNN-slug.md)`.
