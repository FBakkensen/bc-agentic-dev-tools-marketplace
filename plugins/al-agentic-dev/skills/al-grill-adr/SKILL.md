...
Before writing any BC-specific term into `CONTEXT.md` or a domain ADR, declare its citation in chat via `/al-research`: `Researched: <name> → <source path / URL / topic id>`. Vocabulary drift between agents and codebase is what `CONTEXT.md` exists to fix; let the workspace and `/al-research` settle the canonical term. If research fails, keep grilling; do not write the term or the ADR this session. See [voice-contract.md](../../references/voice-contract.md) for prose voice.
