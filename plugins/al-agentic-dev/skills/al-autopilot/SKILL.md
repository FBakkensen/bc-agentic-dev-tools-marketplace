...
2. **One step.** Route the single next step the state names, via the owning skill:
   ...
   | task `blocked` with a replan flag or failed-gate record inline | replan territory → `AUTONOMY STOP REPORT`, resume `/al-steer T-NNN` (detected breaking change: resume is the human's accept-vs-fix at merge instead). Dependency-blocked tasks awaiting their named flip-owner never match this row — `blocked` is the normal scope-time state; the *Replan boundary*'s additive case stays in-loop |
...
- **Write the `decision-log.md` header — absent only.** File already exists (parking makes relaunch the normal flow) → append a one-line run marker under `## Entries` (`### <date> — Relaunch: <prior stop reason>` — a sanctioned third entry shape; the next Decision/Turn entry, not this marker, is the live template later turns mimic) instead; never rewrite the header or duplicate the append marker — the file is append-only. First run: run header (date, feature, goal condition) plus the sentence `This log records decisions; it never selects a seat.`, the per-turn protocol in three lines, the stop-report rule, both entry skeletons from the Decision log section, and the `## Entries` append marker. The header is inert run metadata — nothing closes it on stop, nothing reads it for routing (seat selection per [autonomy-seat.md](../../references/autonomy-seat.md)). The header is the turn-one anchor — a cold start reads the file whole and restores the discipline. Mid-run turns read only the tail, so the entry shape rides the append marker and the previous entry, not the header.
...
Findings → triage (seats above) → fix tasks open `ready` → refine → implement → re-review the diff since the previous round. Rounds cap at 3: lens findings never reach zero, and an unattended loop will chase hygiene nits at full gate cost forever. At cap, remaining correctness findings → stop report; remaining hygiene-level findings → decision-log notes, feature completes. Correctness is never waved through; hygiene never holds the feature hostage. Rounds closed clean → run one final full `/al-build` and surface its gate report in the same turn — the completion clause names that proof and no other step produces it fresh. Fix tasks that landed *after* the `kind=breaking-change` task ran re-flip it `ready` before the run completes: code changed after the baseline check is unvalidated.
...
| **Routes to**        | `/al-refine`, `/al-implement`, `/al-code-review`, `/al-page-script`, `/al-user-verification` (Contract-only verify tasks) per the state table; **parks before walkable `/al-user-verification`** (walk is user-driven); `/al-steer` for in-loop additive insertion only |
