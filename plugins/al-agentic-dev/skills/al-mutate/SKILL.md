...
Written by `/al-implement` to the calling task in `tasks.md`, alongside `**Tests**`. One row per mutation site. `/al-mutate` reads it and writes the result line back.
...
2. Read the calling task ID + `**Mutations**` block when invoked from `/al-implement`; otherwise build a mutation plan from the requested target file or area.
3. Run preflight.
4. For each mutation in the plan, one at a time:
   ...
   - Record one fact per row in the `**Mutations**` block result line.
5. Write the report at `.output/mutation-report/<YYYYMMDD-HHMMSS>.md`.
6. Append the bounded `**Mutations:**` result line to the calling task only when invoked from `/al-implement`.
...
The `tasks.md` line is bounded: one line, the format above. Do not write operator-priority prose, selection rationale, walked-but-skipped paragraphs, or "Lesson:" entries to `tasks.md`. Those belong in the mutation report or stay in the session.
...
- No `tasks.md` restructuring — survivors surface as the bounded result line.
