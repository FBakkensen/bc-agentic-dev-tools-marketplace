...
- **Dependency graph**: Mermaid in a fenced ` ```mermaid ` block earns place when ≥3 tasks AND edges not a single linear chain. Skip for ≤2 tasks or pure linear `Depends on:`. Slice membership already grep-able on `slice=`; graph need not re-encode it.
