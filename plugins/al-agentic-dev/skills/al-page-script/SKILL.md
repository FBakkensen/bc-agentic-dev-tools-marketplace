...
**Layer.** This is the **acceptance layer** of the test pyramid (see [`test-strategy.md`](../../references/test-strategy.md)): a regression guard written *after* the slice, from verify-task scenarios — not a red-first driver. Its oracle is bc-replay's equality/visibility checks, which are **oracle-limited** (a recording can pass against broken code where the platform absorbs the fault). So a red here doesn't get fixed here by default: it **pushes down** to the layer that can pin it. See *Failure classification* below.
...
Once when the slice's `.yml` lands at the committed path. Gate report names slice (slug + `event-model.md` step), scenario count, what user surface the recording exercises (Page action, API endpoint), next handoff `/al-user-verification T-NNN`. Stop shape on routing failures (production bug → verify task `blocked` → `/al-steer` opens the integration fix, unspeakable scenario → `/al-refine` or the exploratory walk) follows [voice-contract.md](../../references/voice-contract.md): one stop line naming scenario / step / observed-vs-expected, state table (verify task ID, scenarios completed, scenario blocked on), next action.
