...
- Clean per-slice `/al-code-review` evidence exists for this verify task. Missing clean review → **Stop**, `Next: /al-code-review T-NNN`. Page-script is a verification pre-flight artifact, not the code-review gate.
...
- **What flips at end?** File committed at `pagescripts/recordings/<NNN>-<slug>__<slice>.yml`. Verify task `status=` stays `ready-for-verification`. Gate report names `Next: /al-user-verification T-NNN`.
...
- **Unexpected platform dialog (a hang).** The run times out with no error string; `error-context.md` shows an open Confirm (`RecordChangeDialog`: "Your change might update related records…", default focus No). Either the dialog is *expected* → YAML defect, answer it; or it is *triggered by a production AL pattern* (the page key-writes its bound `Rec`, a mis-set `AutoSplitKey`) → production bug, route as above (status unchanged; `/al-steer` opens the integration fix task). The AL fix lands in `/al-implement`, not here.
...
| | |
|---|---|
| **Invoked by**     | user. Suggested by `/al-code-review` per-slice (next-action when verify task `ready-for-verification` and `.yml` missing), `/al-steer` (state-read routing on clean-reviewed verify task with `Verification Plan` and no `.yml`) |
| **Runs after**     | `/al-refine` filled the verify task's `Verification Plan`, `/al-code-review` per-slice preserved the verify task at `ready-for-verification` |
