...
**Citation chain when the rename introduces a BC name from outside the codebase.** Most renames pull from canonical sources already research-backed by their authoring skill (`architecture.html` per `/al-design`, `event-model.html` per `/al-event-model`, `CONTEXT.md` per `/al-grill-adr`, Gherkin per `/al-refine`); do not re-verify those. When the rename target comes from somewhere else (a BC verb the agent picks because "Insert is right for Customer", a BaseApp event subscriber signature aligned to current BaseApp, a System Application procedure call introduced during seam extraction), declare in chat: `Researched: <name> → <source path / URL / topic id, verbatim one-liner>` before the rename lands. Route through `/al-research`; do not reach for the underlying tools directly. **Why**: rename to BC vocabulary is exactly where a confidently-wrong verb (`Create` masquerading as canonical when BC says `Insert`, `Submit` instead of `Post`) corrupts every downstream artifact. The citation chain catches the case the lens itself misses: where the agent's certainty about the "right" BC term is the failure mode.
