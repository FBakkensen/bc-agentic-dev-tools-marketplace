...
2. `provision.ps1` — per-feature freshness refresh (compiler, symbols, ALCops, and the breaking-change baseline when enabled). Installs **both** compiler channels (stable + prerelease) side-by-side and refreshes both to latest every run; `-UpdateCompiler` forces a clean reinstall. It already re-fetches symbols every run; owning the baseline (which advances per release) makes the per-feature re-run the norm, not one-time machine setup.
...
al-build owns **two private side-by-side compilers**, never the global `al` dotnet tool. The global tool is the user's own — their LSP/MCP daily driver, on whatever version (often a prerelease) — and `provision.ps1` must never install, update, uninstall, or invoke it. `Install-ALCompiler` installs both the latest **stable** (`<ToolCacheRoot>/al/stable`) and latest **prerelease** (`<ToolCacheRoot>/al/prerelease`) via `dotnet tool install --tool-path` (the prerelease channel adds `--prerelease`), refreshing both every provision, and runs `Install-ALCops` into each channel's `Analyzers` folder. The two-channel sentinel (`<ToolCacheRoot>/al/sentinel.json`, `schemaVersion: 2`) records each channel's `commandPath` + `alcPath` + `version` and the `stableMajor` / `prereleaseMajor`.

The **build picks the channel by app.json `runtime`**: `Get-RequiredRuntimeMajor` takes the max `runtime` major across all apps (main + test + unit; an app with no `runtime` contributes nothing and is pinned to stable), `Resolve-CompilerChannel` returns `prerelease` only when that exceeds the installed stable major (else `stable`; throws when it exceeds even the prerelease major). The decision rests on the bet that the Tools-package major equals the `runtime` major (held for years: stable `17.x` ↔ runtime `17.0`). `Get-LatestCompilerInfo` resolves the channel **offline** from the sentinel (no NuGet on the gate hot path) and fails loud if the selected channel's compiler is absent on disk — `Invoke-ALBuild` then invokes that channel's `al` by **full path**. The channel is repo-wide: `test.ps1` computes the major once and passes it to every `Invoke-ALBuild` so a single test app at a higher `runtime` pulls the whole build (production app included) onto prerelease. Symbols and the container BC version do **not** follow the channel — that scope is the dev's to align.
...
10. Failure-mode check (fail-loud contract): remove one ALCops DLL from the **stable** channel's Analyzers folder (the channel a runtime-less fixture builds with), re-run the gate, expect the build to throw naming the unresolvable analyzer — never a green compile with reduced coverage. Re-run provision to restore.
...
- **AL Runner stays a global dotnet tool; the AL compiler does not.** `Install-ALRunner` installs `MSDyn365BC.AL.Runner` as a **global** dotnet tool (guarded by `Get-Command al-runner`, skipped when `unitTestApp` is not configured) — that is intentional and unchanged. The AL **compiler** moved off the global slot: `Install-ALCompiler` installs both channels under `--tool-path` (see *Compiler channels* above) and never touches the global `al`. Do not "re-align" the two by moving the compiler back to `--global` — that would clobber the user's own compiler.
