...
Resolution is per app: `<appDir>/.vscode/settings.json` wins, repo-root `.vscode/settings.json` is the shared fallback — so the main app can run the full set including AppSourceCop while test apps run a reduced set (TestAutomationCop, no AppSourceCop). Every app — main, test apps, and the unit-test app — compiles through the analyzer gate in every mode (`-UnitTestOnly` included). AL Runner additionally recompiles the unit-test app internally to run it, but that pass carries no `/analyzer:` args, so the gate's own compile is what enforces lint coverage. No `settings.json` → no analyzers. A requested analyzer that cannot be resolved fails the build — the gate never silently compiles with less lint coverage than the settings ask for. `ALCops.Common.dll` (and `Microsoft.Dynamics.Nav.Analyzers.Common.dll` when no Microsoft analyzer is enabled) is appended automatically when missing from the list. Diagnostic prefixes: `AA` CodeCop, `AW` UICop, `AS` AppSourceCop, `PTE` PerTenantExtensionCop, `AC` ApplicationCop, `DC` DocumentationCop, `FC` FormattingCop, `LC` LinterCop (code-quality subset), `PC` PlatformCop, `TA` TestAutomationCop.
