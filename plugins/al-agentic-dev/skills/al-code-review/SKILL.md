...
|---|---|---|---|
| 1 | **Project compliance** | Changes adhere to `CONTEXT.md` (BC vocabulary, business rules), to the design and domain ADRs under `docs/adr/`, to `architecture.html`'s module map and boundaries, and to the originating task's Gherkin scenarios in `tasks.html` (the code does what the bullets said it would) | both |
