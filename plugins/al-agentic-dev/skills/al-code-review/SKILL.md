...
| 6 | Verify ↔ code alignment | Slice's verify task `Verification Plan` in its file under `tasks/` names surfaces, Roles, Status values, clients, and observable checks; changed code under slice's `slice:` actually exposes those surfaces with those names. Drift caught here means `/al-user-verification` would have failed — preempt before the user spends the time. Push-up check: a `Record: yes` E2E or `Contract` example whose `Contract notes` states no wall and names no seam is a finding — an unjustified push-up (`test-strategy.md`); `Record: no` E2E and `Exploration` charters are not push-ups, never flag them. Skip for backend-only slices (no verify task) | per-slice only |
