...
**Topic caching within one run.** The topic universe is small; the same `setloadfields-placement-before-filters` resurfaces across files. Cache `get_bc_topic` responses within one `/al-implement` task / `/al-refactor` / `/al-code-review` / `al-research` agent invocation; fetch fresh across invocations.
...
- Read by `/al-implement` (write-time construct lookup per the evidence bar in `voice-contract.md`), `/al-refactor` (structural-anti-pattern discipline), `/al-code-review` (per-file consultation + cross-file routing), `al-research` agent (one of the source families, governed by its own topic-recommender discipline).
