...
**MCP absent or init fails** → the evidence bar stays satisfiable: quoted Microsoft Learn passage (constructs and names) or `/bc-standard-reference` (names) per `voice-contract.md`. Do not block on the missing server.
