...
- **Attention panel first.** Everything waiting on the developer — computed from frontmatter alone — or empty-state "nothing waits on you". A `blocked` task shows its `blocked-on:` line verbatim; a task whose status puts it at a user-driven step (`ready-for-verification` → the walk, `ready` → refine) names the skill to invoke. What lives only in chat (a report-only review's fix queue) stays in chat — the panel never claims state the files don't hold.
...
- **Deviation badges.** Each `deviations:` entry renders as a one-line badge on its task card — the standing record of unknowns the agent absorbed without asking. Nothing here demands action; visibility is the point. Rendering rule against badge sprawl: on `done` tasks, collapse to a count that expands on demand; full badges show only on tasks still in flight — the ledger in frontmatter stays append-only either way.
