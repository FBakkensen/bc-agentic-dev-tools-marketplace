...
**R** = reads (DB queries, parameters in, events subscribed to), **P** = pure process (decisions, no DB, no external calls), **W** = writes (`Insert` / `Modify` / `Delete`, telemetry, errors, events published). P is the unit-test surface; `Access = Internal` makes it test-accessible without crossing the external interface. Technical proof has two surfaces: Integration crosses the external interface (R + P + W end-to-end); Unit targets P directly with stubbed R/W collaborators. See [tdd.md](tdd.md), [test-specification.md](test-specification.md), and [testability.md](testability.md).
...
- **Two technical test surfaces, both first-class.** Integration tests cross the external seam via `Access = Public`, exercising R and W. Unit tests live *inside* the module, calling internal procedures directly (especially the P layer). AL's `Access = Internal` is test-accessible from the same app; Microsoft's BaseApp tests do this throughout. `Test Specification` scope (`Unit` or `Integration`) decides which surface each AAA case uses.
