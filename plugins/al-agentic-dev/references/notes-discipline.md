...
- **Survives past `done`** → durable artifact outside the task file: `CONTEXT.md`, domain ADR, `.out-of-scope/`, `.not-yet-specified.md`, commit message, PR description, or side-band reference. The per-task file is branch-scoped; dies when feature merges.
...
| In-scope question that matters but can't be phrased as a decision yet | one line appended to `.not-yet-specified.md` at repo root; graduates to a decision or moves to `.out-of-scope/`, never silently absorbed | `/al-grill-adr`, `/al-design` (append); `/al-steer` (groom); `/al-refine` (scan before speccing) |
