...
- **`tasks/` folder:** per-task-file frontmatter, folder integrity, and proof sections —
  ...
  - **`blocked-on:` lifecycle** → **warn**, two checks: a `status: blocked` task without a `blocked-on:` line flags `block without headline; developer can't see why` — the write belongs in the same Edit as the flip; a `blocked-on:` line on a non-`blocked` task flags `stale block headline; remove-on-unblock missed` (`done` tasks may predate the grammar — do not flag).
